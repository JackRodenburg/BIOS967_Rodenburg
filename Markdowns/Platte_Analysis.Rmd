...
ggplot(carp_count_segment_yearly, aes(x = Year, y = Total_Count, group = Segment, color = Segment)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for readability
```
here i would like to plot the four different species of carps together. 
...
print(paste("The range of body length (mm) for the selected species is from", min_length, "to", max_length))
print(paste("The range of body weight (g) for the selected species is from", min_weight, "to", max_weight))

```
I want this to be smallmouth and largemouth bass
...
#isolates just the carp
lepomis_data_avg <- platte_fish_cleaned %>%
  filter(Species %in% c(720, 722, 724, 726, 730, 738))

lepomis_data_avg <- lepomis_data_avg %>%
  filter(!is.na(Count))

# Summarize the data to get the total count per year (or per month if needed)
lepomis_count_yearly <- lepomis_data_avg %>%
  group_by(Year) %>%
  summarize(Total_Count = sum(Count, na.rm = TRUE)) %>%
  ungroup()

# Plot the catfish count over time
ggplot(lepomis_count_yearly, aes(x = Year, y = Total_Count)) +
  geom_smooth() +  # Add lines connecting the points
  geom_point(color = "blue") +  # Add points
  scale_x_continuous(breaks = unique(lepomis_count_yearly$Year)) +  # Specify x-axis breaks to be each year
  labs(title = "Lepomis Population Over Time",
       x = "Year",
       y = "Total Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for readability
...
sucker_data_avg <- platte_fish_cleaned %>%
  filter(Species %in% c(210, 212, 214))
...
plot <- ggplot() +
  ...
  geom_smooth(data = combined_fish_data[combined_fish_data$Origin == "Nat", ], 
            aes(x = Year, y = TotalCount, group = Origin), 
            color = "blue") +
  scale_x_continuous(breaks = unique(native_fish_yearly$Year)) +  # Specify x-axis breaks to be each year
  geom_point() +

  geom_smooth(data = combined_fish_data[combined_fish_data$Origin == "Non", ], 
            aes(x = Year, y = TotalCount, group = Origin), 
            color = "red") +
  geom_point() +
